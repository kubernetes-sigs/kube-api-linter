...
type ItemListNoMarker []Item

type ConfigMapNoMarker map[string]string

type AtomicStructNoMarker struct {
	Field1 string `json:"field1"`
	Field2 string `json:"field2"`
}
...
type TopologyMarkersFieldTest struct {
	...
	// +listType=map // want `marker "listType": marker should be declared on the type definition of ItemListNoMarker instead of the field`
	// +listMapKey=name // want `marker "listMapKey": marker should be declared on the type definition of ItemListNoMarker instead of the field`
	InvalidListTypeOnItemList ItemListNoMarker `json:"invalidListTypeOnItemList"`

	// Invalid: mapType marker on named type
	// +mapType=granular // want `marker "mapType": marker should be declared on the type definition of ConfigMapNoMarker instead of the field`
	InvalidMapTypeOnConfigMap ConfigMapNoMarker `json:"invalidMapTypeOnConfigMap"`

	// Invalid: structType marker on named type
	// +structType=atomic
	InvalidStructTypeOnAtomicStruct AtomicStructNoMarker `json:"invalidStructTypeOnAtomicStruct"`
