...
func validateEquivalentIdentifiers(
	...
	for i, equivalent := range equivalents {
		...
		if localEquivalents.Has(equivalent) {
			errs = append(errs, field.Duplicate(equivalentPath, equivalent))
			continue
		}

		localEquivalents.Insert(equivalent)

		// Check if this equivalent identifier is already used as a preferred identifier
		if knownPreferredMarkers.Has(equivalent) {
			errs = append(errs, field.Invalid(equivalentPath, equivalent, "equivalent identifier cannot be the same as a preferred identifier"))
			continue
		}

		// Check if this equivalent identifier was already used in another marker's equivalent list
		if knownEquivalentMarkers.Has(equivalent) {
			errs = append(errs, field.Duplicate(equivalentPath, equivalent))
			continue
		}

		knownEquivalentMarkers.Insert(equivalent)
	}
