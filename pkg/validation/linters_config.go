...
func validateSSATagsConfig(stc config.SSATagsConfig, fldPath *field.Path) field.ErrorList {
	...
	case "", config.SSATagsListTypeSetUsageWarn, config.SSATagsListTypeSetUsageSuggestFix, config.SSATagsListTypeSetUsageIgnore:
	default:
		fieldErrors = append(fieldErrors, field.Invalid(fldPath.Child("listTypeSetUsage"), stc.ListTypeSetUsage, fmt.Sprintf("invalid value, must be one of %q, %q, %q or omitted", config.SSATagsListTypeSetUsageWarn, config.SSATagsListTypeSetUsageSuggestFix, config.SSATagsListTypeSetUsageIgnore)))
	}
